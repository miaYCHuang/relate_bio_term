# -*- coding: utf-8 -*-
"""
Created on Tue Apr 19 21:51:08 2022
...
def CountMeta(csv_name):
    ...
    #df = pd.read_csv('./new5K/'+ csv_name +'.csv')
...
#for i in range(len(ICD10)):
    ...
    if  str_err != []:#error
        with open('ERROR_p1.txt', 'a',encoding="utf-8") as f:
            for err in str_err:                
                f.write(err+'\n')
        f.close()
'''
...
#好像用不到
def findAllMeta():
    df = pd.read_csv('labTest50.csv',encoding="utf-8") #wiki_DtoT_100.csv
    print(df)
    data = df['labTest50'].tolist()
        ...
        if isinstance(row, str):
            ...
            for concept in concepts:#一筆CSV的多個matamap data
                ...
                if ConceptType=='ConceptAA':#縮寫與全寫
                    ...
                    #print(preferred,semtypes,cui)
    #print('count',cnt)
    #print(Dic_abbreviation)
    print(search_name,len(search_name))
    print(preferred_lst,len(preferred_lst))
    print(semtypes_lst,len(semtypes_lst))
    print(cui_lst,len(cui_lst))


    df_meta = pd.DataFrame({'search_name':search_name,'preferred':preferred_lst,'semtypes':semtypes_lst,'cui':cui_lst})
    print(df_meta)

    df_meta.to_csv('MetaDrugs.csv',index=0,encoding = 'utf8')
