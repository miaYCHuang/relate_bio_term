...
def parse_args():
	...
	parser.add_argument('--input_corpus', nargs='?', default='./data/cui_all.txt',#default='./data/pubmed_sample'  ./data/pubmed_4-gram.csv  ./data/cui_pubmed.txt ./data/all_4-gram.csv
	...
	parser.add_argument('--output_model', nargs='?', default='./bio/pubmed/bio_all_meta',
	                    help='output of word vector model')

	parser.add_argument('--output_bin', nargs='?', default='./bio/pubmed/bio_all_meta.bin',
	...
	parser.add_argument('--windows', type=int, default=8,#5
	...
	parser.add_argument('--sg', default=1, type=int,
						help='if 1, skip-gram is used, otherwise, CBOW')#cbow = 0
...
def main(args):
	...
	model = FastText(MySentences(new_walks,args.input_corpus), vector_size=args.dimensions, window=args.windows, min_count=args.min_count, workers=args.workers,
					 sg=args.sg, epochs=args.iter)
